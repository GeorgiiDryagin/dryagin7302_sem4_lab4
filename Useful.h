...
//return number as integer, ignores not digits
...
//cut string in words by spaces, and count words
char** line_as_words(char* string, int &number)
{
	char** result = nullptr;
	if (string)
	{
		...
		for (int i = 0; i < strlen(string);)
		{
			...
			int mark_first = 1;		//first word have no space before it
			if (i != 0) mark_first = 0;
			if (j - i + mark_first > 1)
			{
				result = (char**)realloc(result, (number + 1) * sizeof(char*));
				result[number] = (char*)malloc((j - i + mark_first) * sizeof(char));

				mark_first = (mark_first == 1) ? 0 : 1;
				for (int t = 0; i < j; i++, t++)
				{
					result[number][t] = *(string + i + mark_first);
					result[number][t + 1] = 0;
				}
				number++;
			}
